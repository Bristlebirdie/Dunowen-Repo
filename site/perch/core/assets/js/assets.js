...
Perch.UI.Assets	= function()
{
	...
	var init_asset_chooser = function() {
		...
		if (placeholder.length) {
			...
			placeholder.html('<a href="#">'+label+'</a>');

			$('form').on('click', '.ft-choose-asset:not(.assets-disabled) a', function(e){
				e.preventDefault();
				var self = $(this).parent('.ft-choose-asset');
	...
	var init_asset_panel = function() {
		...
		$.get(Perch.path+'/core/apps/assets/async/asset-filter.php', function(r){
	...
	var filter_asset_field = function(container, href)
	{
		...
		$.get(Perch.path+'/core/apps/assets/async/asset-filter.php', current_opts, function(r){
			container.html(r);
	...
	var open_asset_drop = function() {
		...
		drop.animate({height: '163px'}).addClass('open');
